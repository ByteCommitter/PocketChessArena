...
def connection():
    global timer_start, game_result
    t0 = int(time.time())
...
def generate_pgn():
    ...
    pgn_str = f'[Site "Pocket Chess Arena"]\n[White "{home.white_name}"]\n[Black "{home.black_name}"]\n[Result "{pgn_result}"]\n\n' + pgn_str + " " + pgn_result

    img_copy = PhotoImage(file = "./Icons/clipboard.png").subsample(3,3)
    button_pgn_copy = Button(home.frame_pgn, image = img_copy, text = "Copy PGN to clipboard", compound = LEFT, font = ('Comic Sans', 14, 'bold'), bg = constant.LIGHTBGCLR, fg = constant.LIGHTBGTEXTCLR, activebackground = constant.LIGHTBGCLR, command = on_pgn_copy, bd = 0, highlightthickness = 0)
